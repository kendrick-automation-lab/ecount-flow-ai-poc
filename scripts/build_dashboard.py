...
.guide{background:rgba(139,124,255,.08);border:1px solid rgba(139,124,255,.22);border-radius:14px;padding:12px 16px;font-size:12.5px;color:var(--text);line-height:1.7;margin:16px 0 2px;}
.guide b{color:var(--accent);font-weight:500;}
.cap{color:var(--muted);font-size:11.5px;line-height:1.55;margin:-4px 0 12px;}
.cap b{font-weight:500;}
.glos{color:var(--muted);font-size:11px;line-height:1.8;margin-bottom:10px;padding-bottom:9px;border-bottom:1px solid rgba(255,255,255,.07);text-align:left;}
.glos b{color:var(--accent);font-weight:500;}
</style>
</head>
<body><div class="wrap">
<header>
...
<div class="two">
  <div class="card"><h2>💰 미수금 많은 거래처 TOP 5</h2>
    <div class="cap">팔고 아직 못 받은 외상값(미수금)이 큰 거래처 순 — <b>누구부터 회수할지</b> 우선순위.</div>
    <canvas id="recvChart" height="200"></canvas></div>
  <div class="card"><h2>⚙️ 업무별 처리 방식 자동 분류</h2>
    <div class="cap">각 업무 건을 <b style="color:var(--ok)">자동 처리</b>·<b style="color:var(--warn)">사람 확인</b>·<b style="color:var(--bad)">수동 검토</b>·스킵 으로 나눈 결과. ※ <b>실제 실행이 아니라 '분류'만</b> — 실행은 사람 승인.</div>
    <canvas id="scenChart" height="200"></canvas></div>
</div>

<div class="two">
  <div class="card"><h2>🚨 안전재고 미만 품목 — 부족 큰 순</h2>
    <div class="cap">현재고가 안전재고(품절 막으려 유지하는 최소량) 아래인 품목. <b>현재고가 음수면 이미 품절+미출고</b>.</div>
    <table><thead><tr><th>품목코드</th><th>품목</th><th class="r">현재고/안전</th><th class="r">부족</th></tr></thead>
    <tbody>__BREACH_ROWS__</tbody></table></div>
  <div class="card"><h2>🧠 자동 추출된 사내 규칙 (적용 전 담당자 확인)</h2>
    <div class="cap">직원들의 협업툴 대화에서 AI 가 뽑아낸 "이럴 땐 이렇게" 규칙 — 자동화에 반영 전 사람이 검토.</div>
    <ul class="rules">__RULES_HTML__</ul></div>
</div>
...
const actLabel={auto_execute:'자동 처리',request_confirm:'사람 확인 요청',manual_review:'수동 검토',skip:'스킵'};
...
new Chart(document.getElementById('scenChart'),{type:'bar',
  data:{labels:[['① 안전재고','(발주 판단)'],['② 입금매칭','(입금↔청구서)'],['③ 구매입력','(견적→전표)']],
    datasets:acts.map(a=>({label:actLabel[a],backgroundColor:col[a],borderRadius:4,
